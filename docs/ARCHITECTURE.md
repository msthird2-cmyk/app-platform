...
Those four states describe one owner's record, and the owner is part of custody rather than something a caller remembers to supply. `createKeyCustody` requires an authenticated identity and derives its storage address from it, so a second person signing in on the same device addresses their own record and finds it empty rather than reading the first person's. This is an ownership boundary the states did not previously carry: they said what was stored, not whose. Making the owner a required parameter is deliberate — custody constructible without an identity addresses a record shared by everyone, so the unsafe form is removed rather than discouraged. The address is hashed for storage-charset safety and not for secrecy: it keeps any provider's identifier storable, and provides nothing at all against an adversary who can read the keystore. It is deterministic from the identity alone, because an address that changed with some local value would orphan a key whose owner can still reach it — a worse failure than the one it would prevent.
